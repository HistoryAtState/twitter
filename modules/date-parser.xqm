...
(: 
    ...
    - by Joe Wicentowski 
    - TODO: test against http://xqzone.marklogic.com/svn/commons/trunk/dates/date-parser-tests.xqy
:)
...
) as element(date)?
{
	...
	then ()
	   ...
	   (: TODO - Removed try-catch here for eXist compatibility - joewiz :)
			...
			let $today := datetime:format-dateTime(current-dateTime(), "yyyy-MM-dd")
			let $date :=
				if(exists($shortCut))
...
) as xs:string?
{
	...
	then
		...
		then
			...
			then "1900"
			else concat("20", local:string-pad("0", 2 - string-length($year)), $year)
...
) as xs:string?
{
	(
		...
		return concat(local:string-pad("0", 2 - string-length(string($pos))), $pos)
	)[1]
};
...
{
	concat(local:string-pad("0", 2 - string-length($day)), $day)
};
...
) as xs:string
{
	...
	let $hours :=
		if(exists($bits[1]))
		then concat(local:string-pad("0", 2 - string-length($bits[1])), $bits[1])
		else "00"
	let $min :=
		if(exists($bits[2]))
		then concat(local:string-pad("0", 2 - string-length($bits[2])), $bits[2])
		else "00"
	let $sec :=
		if(exists($bits[3]))
		then concat(local:string-pad("0", 2 - string-length($bits[3])), $bits[3])
		else "00"
...
) as xs:string?
{
	...
	return
		if($date = ("t0y", "t+0y"))
		then datetime:format-dateTime(current-dateTime(), "yyyy-MM-dd")
		...
		then ()
			...
			return datetime:format-dateTime($date, "yyyy-MM-dd")
};
...
) as xs:date
{
	...
	let $nextMonth := concat(local:string-pad("0", 2 - string-length($nextMonth)), $nextMonth)
	let $nextDate :=
		xs:date(string-join((
			$nextYear,
			$nextMonth,
...
) as element(number)
{
	...
	else <number type="value">{ concat(local:string-pad("0", 2 - string-length($num)), $num) }</number>
};

declare %private function local:string-pad($s as xs:string, $n as xs:integer) as xs:string {
